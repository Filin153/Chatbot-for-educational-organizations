...
async def check_prepod(id = None, name: str = None, today: bool = False, tomorow: bool = False):
    if id:
        ...
        buttons = schedule_buttons_p
        chedule = await PrepodSchedule(group=name, today=today, tomorow=tomorow).run()
        msg_schedule = chedule.schedule
    else:
        buttons = schedule_buttons_g
        chedule = await GroupSchedule(group=name, today=today, tomorow=tomorow).run()
        msg_schedule = chedule.schedule
