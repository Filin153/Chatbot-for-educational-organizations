...
async def training_departament(call: types.CallbackQuery):
    ...
    if call.from_user.id not in all_id:
        await call.answer(
            'Для входа в учебный отдел нужно сначала авторизоваться как '
...
@dp.message_handler(
    content_types=['text'], state=Register.password_departament
)
async def get_fio(message: types.Message, state: FSMContext):
    password = message.text
    departament_data = (
        db.query(TrainingDepartamentData)
        .filter(TrainingDepartamentData.password == password)
        .all()[-1]
    )
    if departament_data:
        account = (
